// ════════════════════════════════════════
// FLAGS & COLORS — copiado de Baltic Sea News
// Detecta la región/país desde el nombre de la fuente
// ════════════════════════════════════════
const SOURCE_FLAGS = {
    'reuters':   { flag: '🌍', color: '#ff7a00' },
    'afp':       { flag: '🇫🇷', color: '#003189' },
    'tass':      { flag: '🇷🇺', color: '#cc0000' },
    'politico':  { flag: '🇺🇸', color: '#e40000' },
    'lemonde':   { flag: '🇫🇷', color: '#1a1a6e' },
    'guardian':  { flag: '🇬🇧', color: '#005689' },
    'telegraph': { flag: '🇬🇧', color: '#0a0a0a' },
    'aljazeera': { flag: '🇶🇦', color: '#8b1a1a' },
    'bbc':       { flag: '🇬🇧', color: '#bb1919' },
    'dw':        { flag: '🇩🇪', color: '#c8102e' },
    'apnews':    { flag: '🇺🇸', color: '#d0021b' },
    'nyt':       { flag: '🇺🇸', color: '#1a1a1a' },
    'cnn':       { flag: '🇺🇸', color: '#cc0001' },
    'france24':  { flag: '🇫🇷', color: '#f0002a' },
    'xinhua':    { flag: '🇨🇳', color: '#de2910' },
    'spiegel':   { flag: '🇩🇪', color: '#e2001a' },
    'scmp':      { flag: '🇭🇰', color: '#cf0a2c' },
    'elpais':    { flag: '🇪🇸', color: '#d00020' },
};

function getSourceMeta(sourceKey) {
    const key = (sourceKey || '').toLowerCase().replace(/[^a-z]/g, '');
    for (const [k, v] of Object.entries(SOURCE_FLAGS)) {
        if (key.includes(k)) return v;
    }
    return { flag: '🌐', color: '#4a8ff5' };
}
...
class NewsMonitorApp {
    ...
    async loadNews() {
        ...
        try {
            ...
            if (!news || !news.length) { this.showError('No se pudieron cargar noticias. Verifica tu conexión.'); return; }
    ...
    applyFilters() {
        ...
        if (!this.activeRegions.has('all')) f = f.filter(n => n.region && this.activeRegions.has(n.region));
        if (this.searchQuery) f = f.filter(n =>
    ...
    createNewsCard(news) {
        ...
        // ── Obtener flag y color desde la tabla de fuentes
        const meta  = getSourceMeta(news.source);
        const color = news.sourceColor || meta.color;
        const flag  = meta.flag;

        // borde top de color + color de fuente
        ...
        if (news.image && this.currentView !== 'compact') {
            imageHtml = `<img src="${news.image}" alt="" class="news-card-image" onerror="this.style.display='none'">`;
        }

        let locationHtml = '';
        if (news.primaryLocation) {
            locationHtml = `<span class="news-location">📍 ${news.primaryLocation.name}</span>`;
        }
        ...
        card.innerHTML = `
            ...
            <div class="news-card-header">
                ...
                <span class="news-card-flag" title="${news.region || ''}">${flag}</span>
    ...
    showNewsModal(news) {
        ...
        body.innerHTML = `
            ...
            <span style="margin-left:6px">${meta.flag}</span>
            <span class="news-date" style="margin-left:8px">${this.formatDate(news.pubDate)}</span>
            <h2 style="margin:10px 0 8px">${news.title}</h2>
            ${news.image ? `<img src="${news.image}" style="width:100%;margin-bottom:12px;border-radius:6px">` : ''}
            ...
            <div class="modal-actions">
                <button class="modal-btn-open"    data-url="${news.link}">Abrir →</button>
                <button class="modal-btn-extract" data-url="${news.link}">Extraer texto</button>
            </div>`;
